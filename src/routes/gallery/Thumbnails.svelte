<script lang="ts">
	...
	function pointerMove(e: { clientX: any; }) {
	  if (isPointerDown) {
	    let { clientX } = e;
	...
	const onMount = bpWrap => {
	  ...
	  bp.open({
	    ...
	    onOpen: ((container) => {
        bpItems = bp.items
        // container.classList.add('fixed', 'h-full', 'items-center')
      }),
...
{#if opts}
  <!-- border-4 border-black flex justify-center z-50 bg-gray-400 -->
	<div
	...
	>
   <!-- m-auto bg-gray-800 border-2 border-gray-950 -->
		<div class="thumbnail-bp" use:onMount />
		<div
			class="thumbnails w-full"
			transition:fade={{
				easing: cubicOut,
